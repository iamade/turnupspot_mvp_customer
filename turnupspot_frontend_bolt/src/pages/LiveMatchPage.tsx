...
const LiveMatchPage: React.FC = () => {
 ...
 const handleTimerExpired = async () => {
  ...
  const fetchGameState = async () => {
    ...
    setAvailableTeams(response.data.available_teams || []);
      ...
      // If a match just ended and we have a new scheduled match, auto-start it
    if (response.data.current_match?.status === 'SCHEDULED') {
      // Auto-promote scheduled match to in-progress
      await post(
        `/games/${actualGameId}/match/start-scheduled`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
...
const handleStartScheduledMatch = async () => {
  ...
  const handleCoinToss = async () => {
    if (!gameId || !token || !isAdmin) return;
              ...
              <div className="bg-white rounded-lg shadow-md p-6">
                ...
                {availableTeams.length >= 2 ? (
                  <div className="space-y-4">
                    ...
                    {/*  Add button to manually end match (for admins) */}
